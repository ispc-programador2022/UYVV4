...
rutaCsvStorms = './Dev/Tormentas/Tormentas.csv'

# Lectura del csv general
df = pd.read_csv(rutaCsv, sep=',', encoding='utf-8')

# Generar csv de sequia y cargar la columna Entity
df_storms = df.filter(like='storms') # filtrar por drought o sequias
df_storms['Pais'] = df['Entity']   # agregar columna pais
print(df_storms) 

# Lo guardo en un archivo csv
df_storms.to_csv(rutaCsvStorms, index=False, encoding='utf-8')
